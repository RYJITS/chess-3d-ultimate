...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<GameSettings>({
    theme: Theme.LEGO, // Default to Lego
  ...
  return (
    <div className="relative w-full h-screen text-white overflow-hidden bg-slate-900">
      ...
      <div className="absolute top-0 left-0 w-full p-4 flex flex-wrap gap-4 items-center justify-between bg-black/40 backdrop-blur-md border-b border-white/10 z-10">
        <h1 className="text-2xl font-bold text-cyan-400 tracking-widest uppercase">CHESS 3D - FINAL VERSION</h1>
        ...
        <div className="text-lg font-semibold tracking-widest">
            {game.isGameOver() ? (
                <span className="text-red-500">
                    PARTIE TERMINÉE 
                    {game.isCheckmate() ? (turn === 'w' ? ' - LES NOIRS GAGNENT' : ' - LES BLANCS GAGNENT') : ' - MATCH NUL'}
                </span>
            ) : (
                <span className={turn === 'w' ? "text-white" : "text-gray-400"}>
                    TOUR : {turn === 'w' ? "BLANCS" : "NOIRS"} 
                    {game.inCheck() && <span className="text-red-400 ml-2 animate-pulse"> (ÉCHEC)</span>}
                </span>
            )}
        </div>
      </div>
      ...
      <div className="absolute bottom-20 right-4 text-right text-xs text-white/40 pointer-events-none z-0">
         <p>Clic Gauche : Sélection/Déplacement</p>
         <p>Clic Droit : Rotation Vue</p>
         <p>Molette : Zoom</p>
      </div>
